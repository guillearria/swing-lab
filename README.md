...
| `python3 -m research.book` | **The (retiring) real-money book [ARC 5 #12]** — liquidation in flight; after the final real fills land it takes one `book retire` and becomes a TERMINAL, frozen evidence ledger (every command then prints one BOOK CLOSED line; `book.csv`/`book_equity.csv` stay tracked). Until then: `close`/`mark`/`snapshot`/`show` as before. Long-realm personal assets live in the private long-term repo, never here |
...
| `python3 -m research.digest` | **THE per-run Telegram message** (HTML), **digest v2 [ARC 5 #12a]** — the 🎯 POOL scoreboard LEADS BOTH legs (n settled long-only · median · beat · Σ pp equal-wt vs own benchmarks · Wilcoxon p · distance to bar, + 🏁 milestone banners at n=10/20/30, the short contrast, and a Δ-since-HEAD change line), then ⚠️ DO-NOW with paste-ready commands + the book. Settle's full 📋 adds orders + bets/movers state + the mix(last 15) mirror line; read's `--slim` 📖 = scoreboard + book + 🟢 system-take + run note. `--notify` prints a delivery VERDICT (`PUSH DELIVERED` / `REJECTED` / `UNCONFIRMED` — re-send ONLY on REJECTED), exits 1 on a non-delivered send, and stamps `research/data/push_log.csv` so a stranded push is flagged by the next delivered message |
...
- **settle** (DAILY): runs `scripts/daily.sh` → scores matured bets + movers (take/skip fwd vs SPY), commits the ledgers (free, deterministic, idempotent); pushes the 📋 digest (+ 🚨 per scored settlement); the 🚨 heartbeat fires only if a step or the digest push failed (NEVER a ✅ success ping).
...
**Notification contract (2026-07-03; v1.2 2026-08-06; v2 2026-08-14 [ARC 5 #12a]): every scheduled run pushes ONE message — SILENCE = BROKEN; a clean weekday = exactly ONE 📋 (settle, the day's full photo) + ONE 📖 (read, the slim morning brief), BOTH led by the 🎯 POOL scoreboard.** 📋 = the settle photo (scoreboard + Δ-since-HEAD + DO-NOW + state), 📖 = the read brief (scoreboard + DO-NOW + book + run note), 🟢 = a counterfactual system take (informational — nothing to execute), 🚨 = attention (a real scored result, or a failed step → check `cron.log`; the heartbeat fires only on failure). ESCALATION beyond the daily two: only 🚨 failures · verdict events (🏁 milestones, PASS-CANDIDATE flip, pass/kill) · spend-gate proposals · anything genuinely needing a human.
